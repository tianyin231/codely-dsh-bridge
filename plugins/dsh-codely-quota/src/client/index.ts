...
const bar = (p: number): string =>
  `<div style="height:5px;border-radius:3px;background:#e9e9ee;overflow:hidden"><div style="height:100%;width:${Math.round(p)}%;background:var(--dsw-accent-color,#4f8cff);border-radius:3px;transition:width .3s"></div></div>`
...
function buildPet(): () => void {
  ...
  function ringState(): void {
    ...
    if (da && Number(da.quota_points) > 0) {
      ...
      const color = remain > 25 ? '#3fb68b' : remain > 10 ? '#e5b94c' : '#e5484d'
