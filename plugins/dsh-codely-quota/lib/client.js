...
window.__ModuleLoader__.load({
  ...
  factory: () => {
    ...
    function bar(p) {
      var w = Math.round(p);
      return '<div style="height:5px;border-radius:3px;background:#e9e9ee;overflow:hidden"><div style="height:100%;width:' + w + '%;background:var(--dsw-accent-color,#4f8cff);border-radius:3px;transition:width .3s"></div></div>';
    }
    ...
    function buildPet() {
      ...
      wrapper.style.cssText =
        "position:fixed;z-index:2147483000;display:none;font-family:var(--dsw-font-ui,sans-serif);filter:drop-shadow(0 4px 14px rgba(0,0,0,.35))";
      ...
      pet.title = "Codely 积分额度（点击展开，按住拖动）";
      pet.style.cssText =
        "width:38px;height:38px;cursor:pointer;border-radius:50%;background:#ffffff;border:1px solid #e4e4ec;display:flex;align-items:center;justify-content:center;touch-action:none;user-select:none;-webkit-user-select:none";
      var R = 14;
      var CIRC = 2 * Math.PI * R;
      pet.innerHTML =
        '<svg width="36" height="36" viewBox="0 0 36 36" style="display:block">' +
          '<circle cx="18" cy="18" r="' + R + '" fill="none" stroke="#e3e3ec" stroke-width="3.5"/>' +
          ...
          '<text data-cq="pct" x="18" y="18" text-anchor="middle" dominant-baseline="central" style="font-size:9px;font-weight:700;fill:#16161c">?</text>' +
        "</svg>";
      ...
      panel.style.cssText =
        "display:none;position:absolute;right:0;bottom:calc(100% + 10px);width:344px;max-width:calc(100vw - 40px);max-height:min(70vh,560px);overflow:auto;border:1px solid #e2e2e9;border-radius:12px;background:#ffffff;box-shadow:0 10px 40px rgba(0,0,0,.18);padding:12px 14px;font-size:12px;line-height:18px;color:#16161c";
      panel.innerHTML =
        '<div style="display:flex;align-items:center;gap:8px">' +
          '<span style="font-weight:700;font-size:13px">Codely 积分额度</span>' +
          '<span data-cq="plan"></span>' +
          '<span style="flex:1"></span>' +
          '<span data-cq="age" style="font-size:10px;color:#72727c"></span>' +
          '<button data-cq="refresh" style="background:#f4f4f7;color:#16161c;border:1px solid #d9d9e2;border-radius:6px;padding:1px 8px;font-size:11px;cursor:pointer">刷新</button>' +
        "</div>" +
        '<div data-cq="banner" style="display:none;margin-top:8px;border:1px solid #f0b6b9;background:#fdecec;border-radius:8px;padding:6px 8px;font-size:11px;color:#c62828"></div>' +
        '<div data-cq="body" style="display:grid;gap:8px;margin-top:10px"></div>' +
        '<div data-cq="foot" style="margin-top:10px;font-size:10px;color:#72727c"></div>';
      ...
      function renderBody() {
        if (!data) {
          bodyEl.innerHTML = lastErr
            ? '<div style="color:#e5484d;font-size:11px">' + esc(lastErr) + "</div>"
            : '<div style="color:#72727c">加载中…</div>';
          return;
        }
        ...
        if (da && Number(da.quota_points) > 0) {
          ch.push(
            '<div style="border:1px solid #e7e7ee;border-radius:10px;padding:8px 10px;background:#f7f7fb">' +
              '<div style="display:flex;justify-content:space-between;font-size:11px;color:#72727c"><span>每日赠送' + (da.quota_timezone ? "（" + da.quota_timezone + "）" : "") + '</span><span data-cq="countdown"></span></div>' +
              '<div style="display:flex;align-items:baseline;gap:6px;margin:4px 0 6px"><span style="font-size:20px;font-weight:700;font-variant-numeric:tabular-nums">' + fmt(da.remaining_points) + '</span><span style="color:#72727c;font-size:11px">/ 每日 ' + fmt(da.quota_points) + " 积分</span></div>" +
              bar(usedPct(da.used_points, da.quota_points)) +
              '<div style="display:flex;justify-content:space-between;font-size:10px;color:#72727c;margin-top:2px"><span>已用 ' + fmt(da.used_points) + "</span><span>窗口 " + pt(da.period_end_at) + "</span></div>" +
            "</div>");
        }
        var bal = "";
        if (bill) {
          var exTag = bill.is_exhausted ? " " + tag("已耗尽", "#e5b94c") : "";
          bal += '<div style="display:flex;align-items:center;gap:6px"><span style="font-size:15px;font-weight:600;font-variant-numeric:tabular-nums">' + fmt(bill.effective_available_points) + "</span>" + exTag +
            '<span style="font-size:11px;color:#72727c">充值积分</span>' +
            (Number(bill.recharged_points) ? '<span style="font-size:10px;color:#72727c">累计 ' + fmt(bill.recharged_points) + "</span>" : "") +
            "</div>";
        }
        ...
        if (bal) ch.push('<div style="border:1px solid #e7e7ee;border-radius:10px;padding:8px 10px;background:#f7f7fb">' + bal + "</div>");
        ...
        if (cp && cp.found && Array.isArray(cp.windows) && cp.windows.length) {
          for (var i = 0; i < cp.windows.length; i++) {
            ...
            wr += '<div style="margin-bottom:6px"><div style="display:flex;justify-content:space-between;font-size:11px"><span>' + label + (w.exhausted ? " " + tag("用尽", "#e5484d") : "") + '</span><span>剩 <b>' + fmt(w.remaining_points) + "</b> / " + fmt(w.quota_points) + "</span></div>" +
              ...
              '<div style="font-size:10px;color:#72727c;margin-top:1px">已用 ' + fmt(w.used_points) + (w.next_boundary_at ? " · 下次刷新 " + pt(w.next_boundary_at) : "") + "</div></div>";
          }
        } else if (cp && !cp.found) {
          wr = '<div style="font-size:11px;color:#72727c">无订阅套餐窗口（免费版）。付费后显示 <b>5小时 / 周 / 月</b> 用量窗。</div>';
        }
        if (wr) ch.push('<div style="border:1px solid #e7e7ee;border-radius:10px;padding:8px 10px;background:#f7f7fb"><div style="font-size:11px;color:#72727c;margin-bottom:4px">套餐窗口限额</div>' + wr + "</div>");

        if (t) {
          var period = data.period || {};
          ch.push('<div style="border:1px solid #e7e7ee;border-radius:10px;padding:8px 10px;background:#f7f7fb">' +
            '<div style="font-size:11px;color:#72727c;margin-bottom:4px">本月 ' + (period.start_date || "?") + " ~ " + (period.end_date || "?") + "</div>" +
            '<div style="display:flex;gap:14px;flex-wrap:wrap">' +
            '<div><b style="font-size:14px;font-variant-numeric:tabular-nums">' + fmt(t.recorded_points) + '</b><div style="font-size:10px;color:#72727c">消耗积分</div></div>' +
            '<div><b style="font-size:14px;font-variant-numeric:tabular-nums">' + fmt(t.settlement_count) + '</b><div style="font-size:10px;color:#72727c">结算次数</div></div>' +
            '<div><b style="font-size:14px;font-variant-numeric:tabular-nums">' + fmt((t.prompt_tokens || 0) + (t.completion_tokens || 0)) + '</b><div style="font-size:10px;color:#72727c">总令牌</div></div>' +
            "</div></div>");
        }
        bodyEl.innerHTML = ch.join("") || '<div style="color:#72727c;font-size:11px">暂无数据</div>';
        if (lastErr) {
      ...
      function refreshQuota(force) {
        ...
        return fetch(API + "/quota" + (force ? "?force=1" : ""), { cache: "no-store" })
          ...
          .then(function (j) {
            if (!j || !j.ok) throw new Error(j && j.error ? j.error : "HTTP " + (j && j.status ? j.status : "?"));
            data = j.data;
            lastOk = Date.now();
            lastErr = "";
            ringState();
      ...
      return function dispose() {
        ...
        pet.removeEventListener("pointerdown", null);
        pet.removeEventListener("pointermove", null);
        pet.removeEventListener("pointerup", null);
        pet.removeEventListener("pointercancel", null);
        pet.removeEventListener("click", null);
      };
    }
